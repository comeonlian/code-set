...
public class LuceneDemo01 {
	...
	public static void main(String[] args) throws Exception{
		 ...
		 * directory, IndexWriterConfig conf) directory:目录对象,也可以是FSDirectory 磁盘目录对象
		 * conf:写入对象的控制
		 */
		...
		/*
		 * 参数说明 public Field(String name, String value, FieldType type) name : 字段名称
		 * value : 字段的值 store : TextField.TYPE_STORED:存储字段值
		 */
		doc.add(new Field("name", "lianliang", TextField.TYPE_STORED));
		doc.add(new Field("address", "中国深圳", TextField.TYPE_STORED));
		doc.add(new Field("dosometing", "I am learning lucene ", TextField.TYPE_STORED));
		writer.addDocument(doc);
		// 创建第二个doc对象
		doc = new Document();
		doc.add(new Field("name", "zhangsan", TextField.TYPE_STORED));
		doc.add(new Field("address", "中国北京", TextField.TYPE_STORED));
		doc.add(new Field("dosometing", "Lucenes programmer", TextField.TYPE_STORED));
		writer.addDocument(doc);
		...
		// Query query = new TermQuery(new Term("address", "中国深圳"));
